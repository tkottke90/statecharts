...
export interface SendResult {
  ...
  error?: Error;
  sendid?: string;
  events?: SCXMLEvent[]; // Events generated by the processor
}
...
export class EventIOProcessorRegistry extends EventEmitter {
  ...
  register(processor: EventIOProcessor): void {
    if (this.processors.has(processor.type)) {
      throw new Error(
        `Event I/O Processor with type '${processor.type}' is already registered`,
      );
    }
  ...
  unregister(type: string): boolean {
    ...
    if (processor.cleanup) {
      processor.cleanup().catch(error => {
        this.emit(
          'error',
          new Error(`Failed to cleanup processor '${type}': ${error.message}`),
        );
      });
    }
  ...
  async send(
    ...
    if (type) {
      ...
      if (!processor) {
        return {
          success: false,
          error: new Error(
            `No Event I/O Processor registered for type '${type}'`,
          ),
        };
      }
    } else {
    ...
    if (!processor) {
      return {
        success: false,
        error: new Error(
          `No suitable Event I/O Processor found for target '${target}'`,
        ),
      };
    }
      ...
      if (processor.cleanup) {
        processor.cleanup().catch(error => {
          this.emit(
            'error',
            new Error(
              `Failed to cleanup processor '${processor.type}': ${error.message}`,
            ),
          );
        });
      }
  ...
  async send(
    ...
    try {
      ...
      try {
        ...
        // Generate http.response event for successful responses
        const responseEvent: SCXMLEvent = {
          name: 'http.response',
          type: 'external',
          sendid: event.sendid,
          origin: target,
      ...
      // Generate error.communication event for network/communication errors
      const commErrorEvent: SCXMLEvent = {
...
export class SCXMLProcessor implements EventIOProcessor {
  ...
  async send(
    event: SCXMLEvent,
    target: string /* config: ProcessorConfig */,
  ): Promise<SendResult> {
    try {
      ...
      if (target.startsWith('scxml:')) {
        ...
        // Generate error event for invalid target format
        const errorEvent: SCXMLEvent = {
          name: 'error.communication',
          type: 'platform',
          sendid: event.sendid,
          ...
          invokeid: event.invokeid,
          data: {
            error: `Invalid SCXML target format: ${target}`,
            source: 'scxml',
          },
        };
      ...
      if (!targetEmitter) {
        // Generate error event for target not found
        const errorEvent: SCXMLEvent = {
          name: 'error.communication',
          type: 'platform',
          sendid: event.sendid,
          ...
          invokeid: event.invokeid,
          data: {
            error: `SCXML target '${targetId}' not found`,
            source: 'scxml',
          },
        };
      ...
      // Generate success event (optional - SCXML processor might not need response events)
      // For now, just return success without generating events
      return {
        success: true,
        sendid: event.sendid,
      };
    } catch (error) {
      // Generate error event for unexpected errors
      const errorEvent: SCXMLEvent = {
        name: 'error.communication',
        type: 'platform',
        sendid: event.sendid,
