...
describe('OnExitNode', () => {
  ...
  describe('SCXML compliance', () => {
    ...
    it('should demonstrate usage as executable content in state exit', async () => {
      ...
      expect(result._pendingInternalEvents).toHaveLength(1);
      expect(result._pendingInternalEvents![0].name).toBe('state.exit.complete');
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('should handle complex nested state modifications', async () => {
      ...
      const { user } = result.data as { user: { id: number; session: { startTime: number; endTime: number; active: boolean } } };
    ...
    it('should handle cleanup operations typical in onexit', async () => {
      ...
      const { persistentData } = result.data as { persistentData: { important: string } };
