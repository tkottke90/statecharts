...
const ParamNodeAttr = BaseNodeAttr.extend({
  ...
  data => {
    ...
    const valueSourceCount = [hasExpr, hasLocation, hasContent].filter(
      Boolean,
    ).length;
  ...
  {
    message:
      "Must specify exactly one of: 'expr' attribute, 'location' attribute, or child content",
  },
);
...
export class ParamNode
  extends BaseNode
  implements z.infer<typeof ParamNodeAttr>
{
  ...
  async evaluateValue(state: InternalState): Promise<unknown> {
    try {
      ...
      throw new Error(
        `Failed to evaluate param '${this.name}': ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  ): CreateFromJsonResponse<ParamNode> {
...
export async function collectParamValues(
  ...
  for (const paramNode of paramNodes) {
    try {
      ...
      throw new Error(
        `Failed to collect parameter '${paramNode.name}': ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
