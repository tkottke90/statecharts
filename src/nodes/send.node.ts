 ...
 */
const SendNodeAttr = BaseExecutableNode.schema
  .extend({
  ...
  .refine(
    ...
    {
      message:
        "Must specify either 'event' attribute or 'eventexpr' attribute, but not both",
    },
  )
  .refine(
    data => {
...
export class SendNode
  ...
  implements z.infer<typeof SendNodeAttr>
{
  ...
  constructor(
    { send }: SendNodeType,
    processorRegistry?: EventIOProcessorRegistry,
  ) {
    super(send);
   ...
   */
  private async evaluateProcessorType(
    state: InternalState,
  ): Promise<string | undefined> {
    if (this.type) {
      return this.type;
   ...
   */
  private async collectNamelistData(
    state: InternalState,
  ): Promise<Record<string, unknown>> {
    if (!this.namelist) {
      return {};
  ...
  private async sendEvent(
    ...
    const result = await this.processorRegistry.send(
      event,
      target,
      processorType,
      config,
  ...
  ): CreateFromJsonResponse<SendNode> {
